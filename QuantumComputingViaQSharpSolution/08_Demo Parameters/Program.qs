namespace _08_Demo_Parameters {
    ...
    operation HelloQ() : Unit {
        let iterations = 1000;
        let message = "Starting to measure the output of H gate by running it " + Convert.IntAsString(iterations) + " number of times.";
        let (zeroCount, oneCount) = CountZeroesAndOnes(iterations, message);
        Message("Number of Zeroes: " + Convert.IntAsString(zeroCount));
        Message("Number of Onoes: " + Convert.IntAsString(oneCount));
    }
